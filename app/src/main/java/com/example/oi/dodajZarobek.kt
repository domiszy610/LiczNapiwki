...
//
     option.onItemSelectedListener = object : OnItemSelectedListener {
        ...
        bDodajZarobek.setOnClickListener {
            ...
            val napiwek: Double = try{ nap.toDouble()
                ...
                Toast.makeText(applicationContext,"Proszę podać poprawną wartość napiwku!!", Toast.LENGTH_SHORT).show()
                return@setOnClickListener


            }
            val godziny: Double = try{ lic.toDouble()
                ...
                Toast.makeText(applicationContext,"Proszę podać poprawną wartość godzin!!", Toast.LENGTH_SHORT).show()
                return@setOnClickListener
            ...
            val data = Kalendarz.date
            val data1 = data.toString()
            ...
            if (id_aktywnosci==null){
                ...
                Toast.makeText(applicationContext,"Proszę uzupełnić pole posada!", Toast.LENGTH_SHORT).show()
                return@setOnClickListener
            }
            else if(!nap.isNullOrEmpty() || !lic.isNullOrEmpty() ){

                db.insertOrThrow(Table1Info.TABLE_NAME, null, value_tab1)
                Toast.makeText(applicationContext,"Udało się dodać do bazy danych!", Toast.LENGTH_SHORT).show()
